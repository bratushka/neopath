...
from neopath.entities import Node, MetaEntity
from neopath.exceptions import BadLabels
from neopath.props import Prop


class MetaEntityTests(TestCase):
    """Tests for MetaEntity class."""
    def test_entity_and_prop_name_on_properties(self):
        """Each Prop should get an `entity` and a `prop_name` attribute."""
        class Entity(metaclass=MetaEntity):
            """MetaEntity instance."""
            name = Prop(prop_name='other_name')
            no_name = Prop()

        self.assertEqual(Entity.name.prop_name, 'other_name')
        self.assertIs(Entity.name.entity, Entity)
        self.assertEqual(Entity.no_name.prop_name, 'no_name')
        self.assertIs(Entity.no_name.entity, Entity)


class NodeTests(TestCase):
    """Tests for Node class."""
    def test_labels(self):
        """Labels should come from class name if not set in Meta."""
        class SomeNode(Node):
            """Node subclass."""
        ...
        self.assertEqual(SomeNode.labels, {'SomeNode'})
        self.assertEqual(OtherNode.labels, {'Hello', 'World'})
        with self.assertRaises(AttributeError):
            SomeNode.labels = ('OtherNode', 'SomeNode')
        with self.assertRaises(AttributeError):
            SomeNode.labels = ('OtherNode',)

        node = SomeNode()
        self.assertEqual(node.labels, {'SomeNode'})

        node.labels = ('OtherNode', 'SomeNode')
        self.assertEqual(node.labels, {'OtherNode', 'SomeNode'})
        self.assertEqual(SomeNode.labels, {'SomeNode'})
        ...
        for bad_labels in (
            ...
            with self.assertRaises(BadLabels):
                print(bad_labels)
                node.labels = bad_labels


# class NodeTests(TestCase):
    # def test_neo_attribute(self):
    #     """
    #     On class creation the Neo class should be removed if exists
    #     and a neo attribute should be created.
    #     """
    #     class Entity(metaclass=MetaEntity):
    #         """Doesn't have a Neo class inside."""
    #     self.assertTrue(hasattr(Entity, 'neo'))
    #     self.assertFalse(hasattr(Entity, 'Neo'))
    #
    #     class Entity(metaclass=MetaEntity):
    #         """Has a Neo class inside."""
    #         class Neo:
